...
exports.removeCommentByCommentId = (commentId) => {
  return checkValueExists(commentId, 'comment_id', 'comments')
  .then(() => {
    return db.query(
      `
...
exports.updateCommentsByCommentId = (commentId, incVotes, bodyKeys) => {
  if (bodyKeys.length === 0) {
    return Promise.reject({status: 400, msg: "bad request must include inc_votes value"});
  } else {
    return db
      .query(
        `
      ...
      .then(({ rows }) => {
        if (rows.length === 0) {
          return Promise.reject({status: 404, msg: "comment_id doesnt exist"});
        } else {
